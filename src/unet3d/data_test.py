...
def write_image_data_to_file(image_files, data_storage, image_shape, n_channels, affine_storage,
...
def add_data_to_storage(data_storage, affine_storage, subject_data, affine, n_channels, truth_dtype):
    data_storage.append(np.asarray(subject_data[:n_channels])[np.newaxis])
    affine_storage.append(np.asarray(affine)[np.newaxis])


def write_data_to_file(testing_data_files, out_file, image_shape, truth_dtype=np.uint8, subject_ids=None,
    ...
    """
    n_samples = len(testing_data_files)
    n_channels = len(testing_data_files[0]) # - 1 # Number of modality? Azam
    ...
    write_image_data_to_file(testing_data_files, data_storage, image_shape,
